...
| 方法名 | 说明                                                         | 参数                  |
| ------ | ------------------------------------------------------------ | --------------------- |
| use    | 可以用于注册 type枚举类型，第一个参数传入枚举类型名称，第二个注册函数需要返回一个 handler 方法，第三个为配置项 handler 可以访问，handler this 会被重新指向为 table， 传参 （h, row, column） 需要返回 vNode 类型，还可以处理创建 table 列不同阶段 | name,install, options |
